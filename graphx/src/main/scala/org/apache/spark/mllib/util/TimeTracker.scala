...
private[mllib] class TimeTracker extends Serializable {
  ...
  def getNanoseconds(timerLabel: String): Double = {
    if (totals.contains(timerLabel)) {
      return totals(timerLabel)
    } else {
      throw new RuntimeException(s"Timer with label $timerLabel not created or completed")
    }
  }

  def getSeconds(timerLabel: String): Double = {
    return getNanoseconds(timerLabel) / 1e9
  }
