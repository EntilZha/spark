 ...
 */
@DeveloperApi
abstract class LDAModel private[topicmodeling] {
...
class LocalLDAModel private[topicmodeling] (
  ...
  override def vocabSize: Long = topics.numRows

  override def topicsMatrix: Matrix = topics

  override def describeTopics(maxTermsPerTopic: Long): Array[Array[(Double, String)]] = {
    ...
    val topicSummary = Range(0, k).map { topicIndex =>
      ...
      topic.toArray.zipWithIndex.sortBy(-_._1).take(maxTermsPerTopic.toInt)
    }.toArray
 ...
 */
@DeveloperApi
class DistributedLDAModel private[topicmodeling] (
   ...
   */
  def topicDistributions: RDD[(Long, Vector)] = state.topicDistributions()
